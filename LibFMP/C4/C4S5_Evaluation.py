...
def measure_PRF_sets(I, I_ref_pos, I_est_pos, details=False):
    ...
    if details:
        ...
        print('P = %0.3f'%P, ';  R = %0.3f'%R, ';  F = %0.3f'%F)
    return P, R, F
...
def evaluate_pairwise(I_ref, I_est):
...
def plot_matrix_label(M, X, color_label=None, title='', clim=[0,1],
                      figsize=(3, 3), cmap='gray_r', fontsize=8, print_labels=True):
    """Plot matrix and label sequence
    Notebook: C4/C4S5_Evaluation.ipynb"""
    fig, ax = plt.subplots(2, 3, gridspec_kw={'width_ratios': [0.1, 1, 0.05],
                'wspace': 0.2, 'height_ratios': [1, 0.1]}, figsize=figsize)
    ...
    plot_seq_label(ax[0,0], X, direction='vertical', color_label=color_label, fontsize=fontsize, print_labels= print_labels);
    return fig, ax
...
def plot_boundary_measures(B_ref, B_est, tau, figsize=(8, 2.5), details=False):
    ...
    fig, ax = plt.subplots(3, 2, gridspec_kw={'width_ratios': [1, 0.02],
            'wspace': 0.2, 'height_ratios': [1, 1, 1]}, figsize=figsize)

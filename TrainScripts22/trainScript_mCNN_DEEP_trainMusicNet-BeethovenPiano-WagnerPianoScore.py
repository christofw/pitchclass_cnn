...
import random
from customModels import musicalCNN_Jo_lastConv_maxPool_LN_deep


########### MODEL ######################################################
model = musicalCNN_Jo_lastConv_maxPool_LN_deep(B=3, 
                       C=5, 
                       L=75, 
                       numOctaves=6, 
...
modelName = datetime.datetime.now().strftime(format = "%Y-%m-%d_%H-%M-%S") + 'mCNN_DEEP_trainMusicNet-BeethovenPiano-WagnerPianoScore'
###################################################################################
numValFiles = 100
